...
WhileLoopInvariantCodeMotion::TryHoistingInvariantInstructionsFromWhileBody(
  ...
  //还有一个案子我们知道怎么处理。如果循环条件只查看元组的一个元素，并且循环体将该元素设置为常量，则有两个选项：

  //1）在该常量上计算条件将返回true。在这种情况下，循环要么执行0次，要么是一个无限循环，这取决于init值。

  //2）根据该常量计算条件返回false。在这种情况下，循环执行0次或1次，具体取决于init值。这意味着，不管初始值是多少，行程计数的上限都是1。

  //检查条件是否依赖于单个参数，并找出哪个参数
  ...
  //map中包含可以被提出的指令，但是单独提出不会加速，该map中的把循环不变式放到提出式中
  flat_hash_set<HloInstruction*> unhoisted_invariant_instructions;
  ...
  if (unhoisted_invariant_instructions.empty() && !hoist_constants_) {
    ...
    // state tuple.
    //在while循环中没有明显的循环不变元素。理论上，这个前提条件太强了——我们可以使用一些代码，例如在while状态下置换元素，但是在每次迭代中使用select来选择相同的值。

    //如果要求提出常量，即使在while状态元组中找不到任何循环不变的值，也需要扫描while主体中的常量。
    return false;
  }
