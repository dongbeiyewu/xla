...
class XlaBuilder {
 ...
 private:
  ...
  //可变标记的数量，接收tokens生成tokens。
  //token是原始类型。可以线程化用来强制排序
  //afterall可以用作tokens的连接，以便在设置操作之后对操作进行排序。
  ...
  friend XlaOp AllGather(XlaOp operand, int64 all_gather_dimension,
   ...
   //operand：数组或数组的非空元组以减少复制
   //computation：简化computation
   //replica_groups：进行简化的组
   //channel_id:用于跨模块通信的可选通道ID
   //当operand为数组的元组时，对元组的每个元素执行全部归约。
   //replica_groups是在其间执行缩减操作的副本组的列表（可以使用来检索当前副本的副本ID ReplicaId）
   //replica_groups必须为空（在这种情况下，所有副本都属于一个组），或者必须包含与副本数量相同数量的元素。
   //channel_id用于跨模块通信：只有all-reduce 具有相同channel_id功能的操作才能相互通信。
  friend XlaOp AllReduce(XlaOp operand, const XlaComputation& computation,
                         absl::Span<const ReplicaGroup> replica_groups,
                         const absl::optional<ChannelHandle>& channel_id,
                         const absl::optional<Shape>& shape_with_layout);
    //operand:n维输入数组
    //split_dimension:间隔中的一个值，该值命名操作数所沿的维[0,n)
    //concat_dimension:间隔中的一个值，该值命名连接分割块的维度[0,n)
    //split_count:参与此操作的核心数。如果 为空，则应为副本数；否则为0。否则，这应该等于每个组中的副本数。replica_groups
    //replica_groups:每个组包含一个副本ID列表。
    //AllToAll是一种将所有内核的数据发送到所有内核的集体操作
    //分散阶段。在每个内核上，操作数split_count 沿着分为多个块split_dimensions，并且这些块分散到所有内核，例如，第i个块被发送到第i个内核。
    //收集阶段。每个核心沿着串联连接接收到的块 concat_dimension。
